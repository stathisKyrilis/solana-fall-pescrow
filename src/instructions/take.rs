...
pub fn process_take_instruction(
    ...
    let [
        taker, maker, mint_a, mint_b, escrow_account, vault,
        taker_ata_a, taker_ata_b, maker_ata_b,
        system_program, token_program, _associated_token_program @ ..
    ] = accounts else {
    ...
    // Copy out, then DROP the RefMut before any CPI.
    ...
    if Address::from(derived_escrow) != *escrow_account.address() {
    ...
    // CPI 1: Bob pays Alice 100 B. Taker signed → invoke()
    ...
    // CPI 2: vault pays Bob all A. PDA must sign → invoke_signed
    pinocchio_token::instructions::Transfer {
        from: vault,
        to: taker_ata_a,
        authority: escrow_account,
        multisig_signers: &[] as &[&AccountView],
        amount: vault_amount, // actual vault balance, not amount_to_give
    ...
    // Escrow is owned by *this* program — close by hand. Lamports first or the runtime screams "unbalanced".
